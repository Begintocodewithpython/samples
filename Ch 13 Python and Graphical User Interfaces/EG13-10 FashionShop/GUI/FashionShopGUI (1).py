...
class FashionShopGUI:
    ...
    def main_menu(self):
        ...
        tagSearchButton.grid(sticky=E, row=0,column=0)
        self.__tagSearchEnty = Entry(root,width=60)
        self.__tagSearchEnty.grid(sticky=W,row=0,column=1)


        self.__stock_item_selector = StockItemSelector(root,self)
        self.__stock_item_selector.frame.grid(row=1, column=0, rowspan=2)
        self.__stock_item_selector.populate_listbox(self.__shop.find_matching_with_tags(self.__search_set))

        self.__stock_item_editor = StockItemEditor(root)
        self.__stock_item_editor.frame.grid(row=1,column=1)
        ...
        button_frame.grid(sticky=E,row=2, column=0, columnspan=2)
