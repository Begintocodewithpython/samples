# EG11-13 Fashion Shop with Tags
...
class StockItem(object):
    ...
    def __init__(self, stock_ref, price, color, location, tags):
...
class Dress(StockItem):
    ...
    def __init__(self, stock_ref, price, color, pattern, size, location,tags):
        if StockItem.show_instrumentation:
            print('** Dress __init__ called')
        super().__init__(stock_ref=stock_ref, price=price,
                         color=color, location=location, tags=tags)
...
class Trousers(StockItem):
    ...
    def __init__(self, stock_ref, price, color, pattern, length, waist, location, tags):
        if StockItem.show_instrumentation:
...
class Jeans(Trousers):
    ...
    def __init__(self, stock_ref, price, color, pattern, length, waist, style, location,tags):
        if StockItem.show_instrumentation:
            print('** Jeans __init__ called')
        super().__init__(stock_ref=stock_ref, price=price,
                         color=color, pattern=pattern, length=length,
                         waist=waist, location=location, tags=tags)
        self.style = style
...
class Hat(StockItem):
    ...
    def __init__(self, stock_ref, price, color, size, location,tags):
        if StockItem.show_instrumentation:
...
class Blouse(StockItem):
    ...
    def __init__(self, stock_ref, price, color, size, style, pattern, location,tags):
        if StockItem.show_instrumentation:
            print('** Blouse __init__ called')
        super().__init__(stock_ref, price, color, location,tags)
        self.size = size
        self.style = style
        self.pattern = pattern
        self.Hat_version = 1
...
class FashionShopShellApplication:
    ...
    def create_new_stock_item(self):
        ...
        def get_stock_details():
            ...
            result['tags'] = FashionShopShellApplication.get_tag_set_from_text(tag_string)
            return result
...
What kind of item do you want to add: '''
        ...
        if item == 1:
            ...
            stock_item = Trousers(stock_ref=stock_details['stock_ref'],
                                  price=stock_details['price'],
                                  color=stock_details['color'],
                                  location=stock_details['location'],
                                  tags=stock_details['tags'],
    ...
    def do_tag_filter(self):
        ...
        search_tags = FashionShopShellApplication.get_tag_set_from_text(tag_string)
...
ui = FashionShopShellApplication('tagsfashionshop.pickle')
ui.main_menu()
